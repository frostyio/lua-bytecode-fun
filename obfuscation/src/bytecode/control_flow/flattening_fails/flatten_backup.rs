...
pub fn has_register(instr: &Instruction, reg: u8) -> Option<Op> {
	match instr.1 {
		Instr::NewTable(_, b, c)
		| Instr::Concat(_, b, c) => {
			if b.0 == reg { return Some(Op::B) };
			if c.0 == reg { return Some(Op::C) };
		}
		Instr::Move(_, b)
		| Instr::LoadNil(_, b) => {
			if b.0 == reg { return Some(Op::B) };
		...
		Instr::SetTable(_, b, crk) => {
			if let RegKst::R(rb) = b {
				if rb.0 == reg { return Some(Op::B) };
			}
			if let RegKst::R(rb) = crk {
				if rb.0 == reg { return Some(Op::C) };
			}
		}
...
pub fn update_mode(mode: &mut Opmode, val: Op, func: &dyn Fn(u32) -> u32)  {
	match mode {
		Opmode::iABC(a, b, c) => {
			match val {
				Op::A => *a = func(*a),
...
fn update_register_offsets(closure: &mut Proto, offset: &mut usize) {
...
pub fn pre_flatten_closures(c: &Proto, register_offset: &mut usize) -> Proto {
	...
	let mut upval_closure_map: HashMap<u8, u8> = HashMap::new();
		...
		match &mut instr.1 {
			Instr::Closure(_, b) => {
				...
				proto 0:
					// an empty proto
				proto 1:
					GetUpval(block_pointer_r, 0) // gets the upvalue which is proto 0, anything referencing block_pointer_r in this closure is referenced 
										// to the upval's original register (@ point a), which is also block_pointer_r
				closure 0:
					Closure(block_pointer_r, 0)
					Closure(Reg(1), 1)
					Move(block_pointer_r, block_pointer_r) // the upvalue referencing proto 0 (point a)
				...
				*/
				...
				let mut upval_references: HashMap<u8, u8> = HashMap::new();
					...
					match inst.1 {
						Instr::GetUpval(a, b) => {
							if let Some(upval) = upval_map.get(&(b as u8)) {
								...
								upval_references.insert(a.0, *upval);
								println!("getupval, in {}, pointing to {upval}", a.0);
							}
						},
						Instr::SetUpval(..) => todo!(),
						_ => {}
				...
				// go through instructions once more and replace instructions that reference the upvalue register with the actual register
				// for (register, original_reg) in upval_references {
				// 	for inst in &mut new_closure.instructions {
				// 		if let Some(op) = has_register(inst, register) {
				// 			let mut mode = &mut inst.2;
				// 			update_mode(&mut mode, op, &|_| original_reg.into());
				// 			println!("function referenced upval {:?}, re-pointing to original reg {original_reg}", inst);
				// 			inst.1 = Instr::from_opmode(inst.0, inst.2);
				// 		}
				// 	}
				// }
			},
...
pub fn flatten(flat_ctx: &mut Context, c: &Proto) -> u32 {
	...
	let mut register_offset: usize = 2; // the inital block_pointer_r offset + function return queue 
	...
	for closure_n in (0..closure_id).rev() {
			...
			while let Some(instr_pt) = iter.next() {
				...
				match inst.1 {
					...
					Instr::Move(a, b) => {
						...
						println!("contains function at register {}? {}", b.0, contains);
						if contains {
							let value = *function_registers.get(&(b.0 as u32)).unwrap();
							function_registers.insert(a.0 as u32, value);
							println!("m: setting closure at reg {} pointing at {}, b: {}", a.0, value, b.0);
						}
					}
					Instr::SetGlobal(a, _b) => {
						function_registers.remove(&(a.0 as u32)); // might be wrong?
					}
		...
		// return to where
		let i1 = flat_ctx.get_or_add_constant(Constants::Number((this_id + (blocks.len() as i32)) as f64));
		flat_ctx.add_instruction(flat_ctx.get_max_ip(), 
			// ip is located in block_pointer_r, and we're using this instruction pointer as an instruction pointer jump sorta
		 Instruction::new(Opcode::Eq, Instr::BinCondOp(false, RegKst::R(block_pointer_r), BinCondOp::Eq, RegKst::K(Kst(256 + i1))))
		);
		flat_ctx.add_instruction(flat_ctx.get_max_ip(), Instruction::new(Opcode::Jump, Instr::Jump(block_pointer_r, 9)));
		
		flat_ctx.add_instruction(flat_ctx.get_max_ip(), Instruction::new(Opcode::Len, Instr::UnOp(queue_4, UnOp::Len, Reg(1))));
		let zero = flat_ctx.get_or_add_constant(Constants::Number(0f64));
		flat_ctx.add_instruction(flat_ctx.get_max_ip(), 
		 Instruction::new(Opcode::Lt, Instr::BinCondOp(false, RegKst::K(Kst(256 + zero)), BinCondOp::Lt, RegKst::R(queue_4)))
		);
		flat_ctx.add_instruction(flat_ctx.get_max_ip(), Instruction::new(Opcode::Jump, Instr::Jump(block_pointer_r, 6)));

		//
		flat_ctx.add_instruction(flat_ctx.get_max_ip(), Instruction::new(Opcode::GetTable, Instr::GetTable(block_pointer_r, Reg(1), RegKst::K(Kst(256 + table_kst4.0)))));
		flat_ctx.add_instruction(flat_ctx.get_max_ip(), Instruction::new(Opcode::GetGlobal, Instr::GetGlobal(queue_1, table_kst)));
		flat_ctx.add_instruction(flat_ctx.get_max_ip(), Instruction::new(Opcode::GetTable, Instr::GetTable(queue_1, queue_1, RegKst::K(Kst(256 + table_kst2.0)))));
		flat_ctx.add_instruction(flat_ctx.get_max_ip(), Instruction::new(Opcode::Move, Instr::Move(queue_2, Reg(1))));
...
impl Flatten {
	...
	pub fn append_end_flatten_statement(&mut self, max_block: u32) {
		...
		ctx.add_instruction(0, Instruction::new(Opcode::NewTable, Instr::NewTable(Reg(1), block_pointer_r, block_pointer_r)));

		ctx.add_instruction(0, // load the entry / current point
		Instruction::new(Opcode::LoadK, Instr::LoadK(block_pointer_r, Kst(entry)))
		);

		let binop_pt = ctx.add_instruction(2, // GE than 0
